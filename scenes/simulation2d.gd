...
func draw_orbits():
	...
	var reference_body_initial_position = Vector2.ZERO
	...
	for step in range(num_steps):
		var reference_body_position = virtual_bodies[reference_frame_index].position if relative_to_body else Vector2.ZERO
		# Update velocities
...
#				line_renderer.points = draw_points[body]
				var local_pos = body.global_transform.affine_inverse() * (draw_points[body][i])
				draw_points[body][i] = local_pos
				line_renderer.points = draw_points[body]
