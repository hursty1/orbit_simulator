...
func _process(_delta):
	for body in bodies:
		var acceleration : Vector2 = CalculateAcceleration(body.position, body)
...
func CalculateAcceleration(point: Vector2, ignoreBody):
	var acceleration : Vector2 = Vector2.ZERO
	for body in bodies:
		if (body != ignoreBody):
			var sqrDst : float = (body.position - point).length_squared()
			var forceDir : Vector2 = (body.position - point).normalized()
